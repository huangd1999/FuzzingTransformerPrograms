...
from src.utils.data_utils import make_reverse_fuzzer
print("start")

# /home/hd/miniconda3/lib/python3.11/trace.py
for step in [60,300,600,3600]:
    filename = "./result/reverse/trace_test_cases_"+str(step)+".txt"
    ...
    while True:
        if time.time() - start > step:
            break
        i+=1
        total_coverage = coverage.Coverage(source=['output.reverse'], data_file='./result/reverse/.coverage.total_'+str(step))
        total_coverage.start()
        fuzzer_sample = make_reverse_fuzzer(vocab_size=8, min_length=0, max_length=8)
        print(fuzzer_sample)
        result = reverse.run(fuzzer_sample["inputs"])
        # 停止总体覆盖率收集并保存数据
        total_coverage.stop()
        total_coverage.save()
        ...
        # 初始化trace追踪器
        tracer = trace.Trace(trace=1, count=True,path_covered='reverse.py')

        # 启动trace追踪
        tracer.runfunc(reverse.run, fuzzer_sample["inputs"])  # 使用reverse.run函数并传入data["inputs"]参数

        # 再次运行相同的测试用例
        reverse.run(fuzzer_sample["inputs"])
        ...
        with open(f'./result/reverse/coverage_{i}.json') as f:
            data = json.load(f)
        os.remove(f'./result/reverse/coverage_{i}.json')
        fuzzing_case = data['files']["output/reverse/reverse.py"]['executed_lines']
        num_statements = data['files']["output/reverse/reverse.py"]['summary']['num_statements']
        ...
        if i == 0:
            ...
            with open(f'./result/reverse/total_coverage_{i-1}.json') as f:
                data = json.load(f)
            os.remove(f'./result/reverse/total_coverage_{i-1}.json')
            total_case = data['files']["output/reverse/reverse.py"]['executed_lines']
        # print(result)
        ...
        total_coverage.start()
        # break
    path = f'./result/reverse/trace_test_cases_{step}.json'
